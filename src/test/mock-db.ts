...
export function createMockDb() {
  ...
  return {
    ...
    column: {
      findFirst: vi.fn(async ({ where, orderBy }: { where?: { boardId?: string }; orderBy?: { order: 'asc' | 'desc' } }) => {
      ...
      create: vi.fn(async ({ data }: { data: { name: string; boardId: string; order: number } }) => {
        const column: Column = {
          id: `column-${Date.now()}`,
          name: data.name,
          order: data.order,
      ...
      update: vi.fn(async ({ where, data }: { where: { id: string }; data: { name?: string; order?: number } }) => {
    ...
    task: {
      findFirst: vi.fn(async ({ where, orderBy }: { where?: { columnId?: string }; orderBy?: { order: 'asc' | 'desc' } }) => {
        let filtered = tasks
        if (where?.columnId) {
      ...
      create: vi.fn(async ({ data }: { data: { title: string; description?: string | null; columnId: string; dueDate?: Date | null; order: number } }) => {
        const task: Task = {
          id: `task-${Date.now()}`,
          title: data.title,
      ...
      update: vi.fn(async ({ where, data }: { where: { id: string }; data: { title?: string; description?: string | null; dueDate?: Date | null; order?: number; columnId?: string } }) => {
